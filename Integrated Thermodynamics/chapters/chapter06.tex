...
\begin{definition}[Boltzmann 因子]
    \begin{equation}
        \text{Boltzmann 因子} \equiv e^{-E(s)/kT} = e^{-E(s)\beta},\quad \text{其中} \beta = (kT)^{-1}.
    \end{equation}
\end{definition}
...
\end{equation}
最终我们得到理想气体分子速率的 Maxwell 分布：
\begin{theorem}[Maxwell 分布]
...
\begin{equation}
    \lambda_n=\frac{2L}{n},\quad n=1,2,\dots
\end{equation}
于是：
\begin{equation}
...
\begin{equation}
    ...
    \equiv\frac{L}{l_Q},
\end{equation}
其中 $l_Q=\frac{h}{\sqrt{2\pi mkT}}$ 是量子长度。推广到 3D：
\begin{equation}
    E_{\mathrm{tr}}=\frac{p_x^2}{2m}+\frac{p_y^2}{2m}+\frac{p_z^2}{2m} \implies Z_{\mathrm{tr}}=\sum_s e^{E_{\mathrm{tr}}/kT} = \prod_\mathrm{cyc} \sum_{n_x} e^{E_x/kT} =\left(\frac{L}{l_Q}\right)^3 \equiv \frac{V}{v_Q},
\end{equation}
其中 $V$ 是盒子体积，$v_Q=l_Q^3=\left(\frac{h}{\sqrt{2\pi mkT}}\right)^3$ 是量子体积。
...
\begin{corollary}[理想气体的热力学性质]
    \begin{align}
        ...
        &G=F+\mathcal{P}V=-NkT\left[\ln\left(\frac{VZ_{\mathrm{int}}}{Nv_Q}\right)+1\right]+NkT=-NkT\ln\left(\frac{VZ_{\mathrm{int}}}{Nv_Q}\right)=N\mu.
    \end{align}
\end{corollary}
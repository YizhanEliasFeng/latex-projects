...
\[
P(a<X\le b)=\int_a^b p(z)\,dz = F(b)-F(a).
\]
...
对于连续型随机变量，
\[
\mathrm{Var}(X)=\int_{-\infty}^{+\infty} x^2p(x)\,dx-\left(\int_{-\infty}^{+\infty} xp(x)\,dx\right)^2.
\]

\subsubsection{线性变换下的均值与方差}
...
\subsection{Gauss分布参数的最大似然估计}

绪论中我们推导了学习的目标——最小化K-L散度——本质上是一个最大似然估计（MLE）。现在我们来推导Gauss分布的MLE。
...
其中：
\begin{itemize}
    \item \(y(x,\mathbf{w})\) 是确定性部分，例如线性模型
    \[
    y=w_1x+w_0,
    \]
    或更一般的多项式模型；
    \item \(\varepsilon\sim \mathcal{N}(0,\sigma^2)\) 是噪声。
\end{itemize}
因此条件分布为
\[
p(t\mid x,\mathbf{w})\sim \mathcal{N}(y(x,\mathbf{w}),\sigma^2).
\]

给定训练样本 \(\{(x_i,t_i)\}_{i=1}^n\)，最大化条件对数似然：
\[
\max_{\mathbf{w}} \sum_{i=1}^n \log p(t_i\mid x_i,\mathbf{w}).
\]

在Gauss噪声假设下，这等价于最小化
\[
\frac{1}{n}\sum_{i=1}^n \left(t_i-y(x_i,\mathbf{w})\right)^2,
\]
即\textbf{最小化均方误差（MSE）}。

所以：\textbf{Gauss噪声假设 \(\Longrightarrow\) 最大似然估计等价于最小二乘回归。}